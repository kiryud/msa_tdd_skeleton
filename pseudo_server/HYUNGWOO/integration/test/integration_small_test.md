...
## 테스트 6. 생성 이벤트는 read-model로 매핑된다(successCriterion -> metric/min)

```text
// Given - DealCreatedEvent(dealType=SELLER, successCriterion=(QUANTITY, 50), capacity=100, deadline, regionCode, hostId)
// When  - DealReadModel.from(event)를 호출한다.
// Then  - successCriterion이 펼쳐져 metric=QUANTITY, min=50으로 매핑되고 capacity=100, regionCode도 그대로 매핑되어야 한다.
```
...
## 테스트 9. 사용자 동네(baseLocation)는 regionCode로 정규화된다

```text
// Given - 사용자 서비스의 baseLocation에 매핑 가능한 동네가 주어진다.
// When  - normalizeRegion(baseLocation)을 호출한다.
// Then  - 대응하는 regionCode(예: MAPO_YEONNAM)가 반환되어야 한다.
// Given - 매핑되지 않는 baseLocation이 주어진다.
// When  - normalizeRegion(baseLocation)을 호출한다.
// Then  - UNSUPPORTED_REGION을 반환해야 한다.
```

> baseLocation을 regionCode로 바꿔야 `isRegionMatched`(테스트 8)에 넘길 수 있다. 사용자 서비스가 구조화된 regionCode를 직접 제공하면 이 정규화는 생략 가능하다.

## 테스트 10. 마감 알림 참여자 목록은 metric에 맞는 소스에서 수집된다

```text
// Given - SMEMBERS/주문자 조회가 목으로 대체되어 있다.
// Given - deal.metric=HEADCOUNT
// When  - collectParticipants(deal)을 호출한다.
// Then  - 참여 SET(timedeal:{dealId}:participants)의 SMEMBERS 결과가 반환되어야 한다.
// Given - deal.metric=QUANTITY 또는 AMOUNT
// When  - collectParticipants(deal)을 호출한다.
// Then  - 주문자 userId 목록(findUserIdsByDeal)이 반환되어야 한다.
```

> QUANTITY/AMOUNT 경로는 주문 서비스에 "딜 단위 주문자 조회(findUserIdsByDeal)"가 추가되어야 실행 가능하다(현재 order_service.md는 orderId/userId 기준 조회만 제공).
